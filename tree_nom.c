...
void flechieSpecialNom(char* genre, char* sing_plur){
    t_tree_nom tree = createEmptyTreeNom();
    generateTreeNom((FILE *) NotreFichier, &tree);
    p_letter_node_nom temp;
    int STOP = 1;
    ...
    while(STOP) {
        ...
        if(trouver && temp->list[i].genre[0] != '\0' && temp->list[i].sing_plur[0] != '\0') {
            ...
            while (temp->list[i].nom[cpt] != '\0') {
                printf("%c",
                       temp->list[i].nom[cpt]); // On reconvertie le tableau d'entier en tableau de caractère pour le nom
                cpt++;
            }
            /**
            printf(" : ");
